...
L1（原始观察）: contexts/memory/OBSERVATIONS.md → agent 主动检索
L2（周反思）: contexts/memory/WEEKLY_REFLECTIONS.md → 每周蒸馏
L2.5（长期偏好）: contexts/memory/PREFERENCE_REGISTRY.md → 稳定默认但仍可调整
```
...
再运行一次 reflector 后，检查 `contexts/memory/WEEKLY_REFLECTIONS.md` 与 `contexts/memory/PREFERENCE_REGISTRY.md` 是否被更新。
...
**Q：observer.py / reflector.py 需要什么依赖？**  
A：依赖 `opencode_client.py`（仓库已提供 OpenCode Server 客户端封装）、`requests` 和 `python-dotenv`，以及可访问的 OpenCode Server。
